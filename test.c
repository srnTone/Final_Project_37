...
// assertion แบบไม่ทำให้โปรแกรมหยุดทันที (จะพิมพ์ผลต่อเนื่อง)
void assert_equal_int(int expected, int actual, const char *name) {
    UT_TOTAL++;
    if (expected != actual) {
...
void assert_equal_str(const char *expected, const char *actual, const char *name) {
    UT_TOTAL++;
...
// ============ Non-interactive APIs for Unit Testing ============

// รีเซ็ตฐานข้อมูลในหน่วยความจำ (ใช้ก่อนแต่ละ test)
void resetDB(void) {
...
// เพิ่มข้อมูลแบบตรง (ไม่ถามผู้ใช้) — ใช้ logic เดียวกับ addMode
// return 1 = success, 0 = invalid/duplicate/full
                    ...
                    const char *date_in)
{
    ...
    if (recCount >= MAX_RECORDS) return 0;
    if (!normalizePolicy(num_in, num_norm)) return 0;
...
// นับผลการค้นหาแบบไม่โต้ตอบ (ใช้เงื่อนไขเดียวกับ searchMode)
int searchCountByPolicy(const char *key) {
    int c = 0;
    for (int i = 0; i < recCount; ++i)
        if (containsIgnoreCase(policyNumber[i], key)) c++;
    return c;
}
int searchCountByOwner(const char *key) {
    int c = 0;
    for (int i = 0; i < recCount; ++i)
        if (containsIgnoreCase(ownerName[i], key)) c++;
    return c;
}
int searchCountByModel(const char *key) {
    int c = 0;
    for (int i = 0; i < recCount; ++i)
        if (containsIgnoreCase(carModel[i], key)) c++;
    return c;
}

// ===================== Unit Tests (ADD & SEARCH) =====================

// --- Test: ADD ---
void test_add_normal_success() {
    // Arrange
    resetDB();
    // Act
    int ok = addPolicyDirect("P23", "Alice", "Civic", "2025-5-5");
    // Assert
    assert_equal_int(1, ok, "add normal returns success");
    assert_equal_int(1, recCount, "recCount after add");
    assert_equal_str("P023", policyNumber[0], "policy normalized");
    assert_equal_str("2025-05-05", startDate[0], "date formatted");
}

void test_add_duplicate_rejected() {
    resetDB();
    addPolicyDirect("A1", "Ann", "City", "2025-1-2");            // -> A001
    int ok2 = addPolicyDirect("A1", "Anyone", "Anything", "2025-2-2");
    assert_equal_int(0, ok2, "duplicate policy should be rejected");
    assert_equal_int(1, recCount, "recCount unchanged after duplicate");
}

void test_add_invalid_policy() {
    resetDB();
    int ok = addPolicyDirect("12A", "Bob", "Jazz", "2025-3-3");   // รูปแบบผิด
    assert_equal_int(0, ok, "invalid policy format rejected");
    assert_equal_int(0, recCount, "recCount unchanged when invalid");
}

void test_add_invalid_date() {
    resetDB();
    int ok = addPolicyDirect("B99", "Bee", "Yaris", "2025-13-40"); // วัน/เดือนไม่ถูก
    assert_equal_int(0, ok, "invalid date rejected");
    assert_equal_int(0, recCount, "recCount unchanged when invalid date");
}

void run_unit_tests_add() {
    printf("\n===== UNIT TESTS: ADD =====\n");
    UT_TOTAL = UT_FAIL = 0;
    test_add_normal_success();
    test_add_duplicate_rejected();
    test_add_invalid_policy();
    test_add_invalid_date();
    printf("----------------------------------\n");
    printf("ADD TESTS: %d run, %d failed\n", UT_TOTAL, UT_FAIL);
    printf("==================================\n");
}

// --- Test: SEARCH ---
void prepare_search_dataset() {
    resetDB();
    addPolicyDirect("A1",  "Ann",    "Civic", "2025-1-2");  // A001
    addPolicyDirect("B12", "Boon",   "City",  "2025-5-5");  // B012
    addPolicyDirect("A23", "Anucha", "Civic", "2025-9-9");  // A023
}

void test_search_by_policy_partial() {
    prepare_search_dataset();
    int c = searchCountByPolicy("A0"); // A001, A023
    assert_equal_int(2, c, "search policy partial 'A0'");
}

void test_search_by_owner_case_insensitive() {
    prepare_search_dataset();
    int c = searchCountByOwner("an"); // Ann, Anucha
    assert_equal_int(2, c, "search owner 'an' (case-insensitive)");
}

void test_search_by_model_exact_word() {
    prepare_search_dataset();
    int c1 = searchCountByModel("Civic");
    assert_equal_int(2, c1, "search model 'Civic'");
    int c2 = searchCountByModel("Unknown");
    assert_equal_int(0, c2, "search model 'Unknown' -> 0");
}

void run_unit_tests_search() {
    printf("\n===== UNIT TESTS: SEARCH =====\n");
    UT_TOTAL = UT_FAIL = 0;
    test_search_by_policy_partial();
    test_search_by_owner_case_insensitive();
    test_search_by_model_exact_word();
    printf("----------------------------------\n");
    printf("SEARCH TESTS: %d run, %d failed\n", UT_TOTAL, UT_FAIL);
    printf("==================================\n");
}

// --- เมนู Unit Test ---
void unitTestMenu(void) {
    while (1) {
        int ch = -1;
        printf("\n------ Unit Test ------\n");
        printf("1) Test ADD\n");
        printf("2) Test SEARCH\n");
        printf("9) Run ALL (ADD + SEARCH)\n");
        ...
        if (ch == 0) return;
        else if (ch == 1) run_unit_tests_add();
        else if (ch == 2) run_unit_tests_search();
        else if (ch == 9) { run_unit_tests_add(); run_unit_tests_search(); }
