...
// ---------- Helpers: รีเซ็ต/อัดข้อมูลลงหน่วยความจำ ----------
void resetDB(void) {
    recCount = 0;
    memset(policyNumber, 0, sizeof(policyNumber));
    memset(ownerName,    0, sizeof(ownerName));
    memset(carModel,     0, sizeof(carModel));
    memset(startDate,    0, sizeof(startDate));
}
void add_direct(const char* num,const char* name,
                       const char* model,const char* date) {
    strncpy(policyNumber[recCount], num,   STRLEN-1);
    strncpy(ownerName[recCount],    name,  STRLEN-1);
    strncpy(carModel[recCount],     model, STRLEN-1);
    strncpy(startDate[recCount],    date,  STRLEN-1);
    policyNumber[recCount][STRLEN-1] = '\0';
    ownerName[recCount][STRLEN-1]    = '\0';
    carModel[recCount][STRLEN-1]     = '\0';
    startDate[recCount][STRLEN-1]    = '\0';
...
// ---------- Helper: รันฟังก์ชัน interactive ด้วยสคริปต์อินพุต ----------
char* run_with_io(void (*fn)(void), const char* script) {
    #ifdef _WIN32
    char in_t[]  = "ut_in_XXXXXX";
    char out_t[] = "ut_out_XXXXXX";
    #else
    char in_t[]  = "/tmp/ut_in_XXXXXX";
    char out_t[] = "/tmp/ut_out_XXXXXX";
    #endif
    int in_fd  = mkstemp(in_t);
    int out_fd = mkstemp(out_t);
    if (in_fd < 0 || out_fd < 0) { perror("mkstemp"); exit(1); }

    // เขียนอินพุตจำลอง
    if (write(in_fd, script, (int)strlen(script)) < 0) { perror("write"); exit(1); }
    lseek(in_fd, 0, SEEK_SET);

    FILE *fin  = fdopen(in_fd,  "r");
    FILE *fout = fdopen(out_fd, "w+");
    if (!fin || !fout) { perror("fdopen"); exit(1); }

    int stdin_bak  = dup(fileno(stdin));
    int stdout_bak = dup(fileno(stdout));
    fflush(stdin); fflush(stdout);
    dup2(fileno(fin),  fileno(stdin));
    dup2(fileno(fout), fileno(stdout));

    // เรียกโหมดที่ต้องการทดสอบ
    fn();

    fflush(stdout);
    // คืนค่า io เดิม
    dup2(stdin_bak,  fileno(stdin));
    dup2(stdout_bak, fileno(stdout));
    close(stdin_bak);
    close(stdout_bak);

    // อ่านเอาต์พุตที่ได้มาเป็นสตริง
    fseek(fout, 0, SEEK_END);
    long sz = ftell(fout);
    fseek(fout, 0, SEEK_SET);
    char *buf = (char*)malloc((size_t)sz + 1);
    if (!buf) { perror("malloc"); exit(1); }
    fread(buf, 1, (size_t)sz, fout);
    buf[sz] = '\0';
...
// ---------- Assertions ----------
#define EXPECT_TRUE(expr, msg) do{ if(!(expr)){ \
    fprintf(stderr,"[FAIL] %s:%d: %s\n", __FILE__, __LINE__, msg); exit(1);} }while(0)
#define EXPECT_EQ(a,b) do{ if((a)!=(b)){ \
    fprintf(stderr,"[FAIL] %s:%d: expected %ld == %ld\n", __FILE__, __LINE__, (long)(a), (long)(b)); exit(1);} }while(0)
#define EXPECT_STREQ(a,b) do{ if(strcmp((a),(b))!=0){ \
    fprintf(stderr,"[FAIL] %s:%d: expected \"%s\" == \"%s\"\n", __FILE__, __LINE__, (a), (b)); exit(1);} }while(0)
#define EXPECT_CONTAINS(hay,needle) do{ if(strstr((hay),(needle))==NULL){ \
    fprintf(stderr,"[FAIL] %s:%d: output missing: %s\n", __FILE__, __LINE__, (needle)); exit(1);} }while(0)

// ---------- Tests: addMode ----------
void test_add_success_and_normalization() {
    resetDB();
    // p23 -> P023, วันที่ 2025 5 5 -> 2025-05-05, ตอบ n เพื่อออกจาก add
    char *out = run_with_io(addMode, "p23\nAlice\nCivic\n2025 5 5\nn\n");
    EXPECT_CONTAINS(out, "Added! (P023)");
    EXPECT_EQ(recCount, 1);
    EXPECT_STREQ(policyNumber[0], "P023");
    EXPECT_STREQ(ownerName[0],    "Alice");
    EXPECT_STREQ(carModel[0],     "Civic");
    EXPECT_STREQ(startDate[0],    "2025-05-05");
    free(out);
    unlink("policies.csv"); // ไฟล์ที่ addMode เซฟอัตโนมัติ
}
void test_add_duplicate_policy_rejected() {
    resetDB();
    add_direct("P023","Alice","Civic","2025-05-05");
    // ใส่ p23 ซ้ำ → ควรแจ้ง Duplicate แล้วกด 0 เพื่อ Back ออกจาก add
    char *out = run_with_io(addMode, "p23\n0\n");
    EXPECT_CONTAINS(out, "Duplicate policy number (P023)");
    EXPECT_EQ(recCount, 1); // ไม่เพิ่ม
    free(out);
}
void test_add_invalid_policy_then_ok() {
    resetDB();
    // เริ่มด้วยเลขผิด "23p" → แจ้ง Invalid format → ใส่ p30 (ถูก) แล้วเพิ่มได้
    char *out = run_with_io(addMode, "23p\np30\nBob\nJazz\n2025-6-1\nn\n");
    EXPECT_CONTAINS(out, "Invalid format.");
    EXPECT_CONTAINS(out, "Added! (P030)");
    EXPECT_EQ(recCount, 1);
    EXPECT_STREQ(policyNumber[0], "P030");
    EXPECT_STREQ(startDate[0], "2025-06-01");
    free(out);
    unlink("policies.csv");
}
void test_add_invalid_date_then_back() {
    resetDB();
    // ให้เลข/ชื่อ/รุ่นถูก แต่วันที่ผิด แล้วกด 0 เพื่อ Back ออก ไม่ควรเพิ่ม
    char *out = run_with_io(addMode, "A1\nName\nModel\n2025-15-40\n0\n");
    EXPECT_CONTAINS(out, "Invalid date.");
    EXPECT_EQ(recCount, 0);
    free(out);
}
...
void seed3() {
    resetDB();
    add_direct("P001","Alice","Civic","2025-01-01");
    add_direct("P023","Bob","Jazz","2025-05-05");
    add_direct("B120","Alina","City","2023-12-31");
}
void test_search_by_policy_partial() {
    seed3();
    // เมนู 1 = policy, คีย์ "P0" ต้องเจอ P001 และ P023 แล้ว 0 ออกจากหน้าค้นหา, 0 กลับเมนูหลัก
    char *out = run_with_io(searchMode, "1\nP0\n0\n0\n");
    EXPECT_CONTAINS(out, "Found 2 record(s).");
    EXPECT_CONTAINS(out, "P001");
    EXPECT_CONTAINS(out, "P023");
    free(out);
}
void test_search_by_owner_case_insensitive() {
    seed3();
    // เมนู 2 = owner, คีย์ "ali" ต้องเจอ Alice และ Alina (ไม่แคร์พิมพ์เล็กใหญ่)
    char *out = run_with_io(searchMode, "2\nali\n0\n0\n");
    EXPECT_CONTAINS(out, "Found 2 record(s).");
    EXPECT_CONTAINS(out, "Alice");
    EXPECT_CONTAINS(out, "Alina");
    free(out);
}
void test_search_no_results() {
    seed3();
    // เมนู 3 = car model, คีย์ "xxx" ไม่พบ
    char *out = run_with_io(searchMode, "3\nxxx\n0\n0\n");
    EXPECT_CONTAINS(out, "Found 0 record(s).");
    EXPECT_CONTAINS(out, "(No results.)");
    free(out);
}

// ---------- Test Runner ----------
int main(void) {
    printf("[RUN] unit tests for addMode & searchMode\n");
    test_add_success_and_normalization();
    test_add_duplicate_policy_rejected();
    test_add_invalid_policy_then_ok();
    test_add_invalid_date_then_back();
    test_search_by_policy_partial();
    test_search_by_owner_case_insensitive();
